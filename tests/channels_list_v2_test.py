...
from tests.test_helpers import invalid_token2, invalid_token3
...
def test_channel_list_v2_user_id(clear):
...
def test_channels_list_v2_no_channels(clear):
...
def test_channels_list_v2_list_first_channel_create(clear):
    ...
    # Create a channel and get channel id
...
# Testing when one private channel is created
def test_channels_list_v2_list_first_private_create(clear):
...
def test_channels_list_v2_list_multi_channel(clear):
...
def test_channels_list_v2_list_user_in_no_channel(clear):
...
# join function
def test_channels_list_v2_list_multiple_channels(clear):
    ...
    requests.post(config.url + "channel/invite/v2",
                 json={'token': token3, 
                       "channel_id": channel_response3["channel_id"], 
                       "u_id": auth_user_id2})
    ...
    list_response2 = requests.get(config.url + "channels/list/v2",
                                    params={'token': token2})

    list_response2 = list_response2.json()

    # Get response on channels/list/v2 for user 2
    list_response3 = requests.get(config.url + "channels/list/v2",
                                    params={'token': token3})

    list_response3 = list_response3.json()

    # Get response on channels/list/v2 for user 1
    list_response1 = requests.get(config.url + "channels/list/v2",
                                    params={'token': token1})
